...
struct PNAccessLevelsStruct {
    ...
    // Used to identify object wide access level.
    __unsafe_unretained NSString *object;
    
    // Used to identify concrete user access level.
    __unsafe_unretained NSString *user;
    
    // Used to identify concrete \cloud object user access level.
    __unsafe_unretained NSString *objectUser;
};
...
/**
 Parse cloud \a 'object' access rights information from provided dictionary.
 
 @param objectInformationDictionary
 \a NSDictionary instance which hold information from server about access rights configuration for cloud object(s).
 */
...
+ (BOOL)isResponseConformToRequiredStructure:(PNResponse *)response {
    ...
    if (conforms) {
        ...
        id objects = [accessInformation valueForKeyPath:kPNAccessObjectsKey];
        ...
        conforms = ((conforms && objects) ? [objects isKindOfClass:[NSDictionary class]] : conforms);
            ...
            if (clientsData && conforms) {
                ...
                if (conforms) {
                    ...
                    if (!channels) {
                        ...
                        [clientsData valueForKeyPath:kPNAccessObjectKey];
                    }
        ...
        // Checking whether found information for objects access rights or not.
        else if (conforms && objects) {
            
//            [(NSDictionary *)objects enumerateKeysAndObjectsUsingBlock:^(id objectName, id objectInformation,
//                                                                         BOOL *objectInformationEnumeratorStop) {
//                
//                conforms = (conforms ? (objectName && [objectName isKindOfClass:[NSString class]]) : conforms);
//                conforms = (conforms ? [objectInformation isKindOfClass:[NSDictionary class]] : conforms);
//                if (conforms){
//                    
//                    id accessLevel = [objectInformation valueForKeyPath:kPNAccessLevelKey];
//                    id accessPeriod = [channelInformation valueForKeyPath:kPNAccessRightsPeriodKey];
//                    conforms = ((conforms && accessLevel) ? [accessLevel isKindOfClass:[NSString class]] : conforms);
//                    conforms = ((conforms && accessPeriod) ? [accessPeriod isKindOfClass:[NSNumber class]] : conforms);
//                    checkClients([channelInformation valueForKeyPath:kPNAccessChannelsAuthorizationKey]);
...
- (id)initWithResponse:(PNResponse *)response {
    ...
    if ((self = [super init])) {
        ...
        else if (accessLevel == PNObjectAccessRightsLevel) {
...
- (void)parseObjectAccessInformationFromDictionary:(NSDictionary *)objectInformationDictionary {
    ...
    NSDictionary *objectsInformation = [objectInformationDictionary valueForKeyPath:kPNAccessObjectsKey];
    [objectsInformation enumerateKeysAndObjectsUsingBlock:^(NSString *objectIdentifier, NSDictionary *objectInformation,
                                                            BOOL *objectInformationEnumeratorStop) {
        
        PNSynchronizationChannel *channel = [PNSynchronizationChannel channelForObject:[objectIdentifier stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
        ...
        // Checking whether \a 'object' level access rights has been changes as well or not.
...
- (void)parseClientAccessInformationFromDictionary:(NSDictionary *)clientsInformationDictionary {
    ...
    if (!channelName) {
        ...
        channelName = [clientsInformationDictionary valueForKeyPath:kPNAccessObjectKey];
    }
    PNChannel *channel = [PNChannel channelWithName:[channelName stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
...
- (PNAccessRightsLevel)accessRightsLevelFromString:(NSString *)stringifiedAccessLevel {
    ...
    else if ([stringifiedAccessLevel isEqualToString:PNAccessLevels.object]) {
        
        level = PNObjectAccessRightsLevel;
    }
    else if ([stringifiedAccessLevel isEqualToString:PNAccessLevels.user] ||
             [stringifiedAccessLevel isEqualToString:PNAccessLevels.objectUser]) {
